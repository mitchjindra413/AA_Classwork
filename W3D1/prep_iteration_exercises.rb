...
class Array
  ...
  def bubble_sort(&prc)
    ...
    while !sorted
      ...
      (0...self.length - 1).each do |idx|
        if prc.call(self_copy[idx], self_copy[idx + 1]) == 1
          self_copy[idx], self_copy[idx + 1] = self_copy[idx + 1], self_copy[idx]
          sorted = false
        end
      end
    end
...
# p [1, 2, 3, 4, 5].bubble_sort { |num1, num2| num2 <=> num1 }
...
def substrings(string)
  ...
  (0...string.length).each do |idx1|
    (0...string.length).each do |idx2|
      results << string[idx1..idx2] if idx2 >= idx1
    end
  end
...
def subwords(word, dictionary)
  subtrings_array = substrings(word)
  
  subtrings_array.select { |ele| dictionary.include?(ele)}
end
